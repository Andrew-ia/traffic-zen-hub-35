...
export async function ensureGrowthMetricsSchema() {
  if (schemaReady) return schemaReady;
  schemaReady = ensureRuntimeSchema(
    'Mercado Livre growth metrics',
    {
      tables: ['ml_item_visits_daily', 'ml_account_metrics_daily', 'ml_sales_hourly'],
      columns: {
        ml_item_visits_daily: ['workspace_id', 'item_id', 'visit_date', 'visits', 'created_at', 'updated_at'],
        ...
        ml_sales_hourly: ['workspace_id', 'metric_hour', 'orders', 'units', 'revenue', 'created_at', 'updated_at'],
      },
    },
    async () => {
      const pool = getPool();
