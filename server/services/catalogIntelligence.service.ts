...
export class CatalogIntelligenceService {
    ...
    async analyzeCatalog(workspaceId: string, accessToken: string): Promise<CatalogIntelligenceResult> {
        try {
            ...
            const allUserProducts = await this.getUserProducts(workspaceId, accessToken);

            // LIMITAR a 20 produtos para evitar timeout em grandes catálogos durante a fase beta
            // Futuramente, mover isso para um job em background (queue)
            const userProducts = allUserProducts.slice(0, 20);

            console.log(`[Catalog Intelligence] Iniciando análise de ${userProducts.length} produtos (de ${allUserProducts.length} totais)...`);

            // Analisar produtos em paralelo com limite de concorrência
            const catalogData: ProductCatalogData[] = [];
            const CONCURRENCY_LIMIT = 5;

            for (let i = 0; i < userProducts.length; i += CONCURRENCY_LIMIT) {
                const chunk = userProducts.slice(i, i + CONCURRENCY_LIMIT);

                const promises = chunk.map(productId =>
                    this.analyzeProductInCatalog(productId, accessToken)
                        .catch(err => {
                            console.warn(`Erro ao analisar produto ${productId}:`, err);
                            return null;
                        })
                );
                ...
                // Filtrar nulos (erros) e adicionar ao array principal
                results.forEach(result => {
                    if (result) catalogData.push(result);
                });

                console.log(`[Catalog Intelligence] Processados ${Math.min(i + CONCURRENCY_LIMIT, userProducts.length)}/${userProducts.length}`);
            }
            ...
            return {
                ...
                category_analysis: {
                    ...categoryAnalysis,
                    total_products: allUserProducts.length // Manter total real para informar o usuário
                },
